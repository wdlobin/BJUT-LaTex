...
`cheadertitle`、`ccoverdegree`、`ecoverdegree`、`cinnertitle` 和 `cstudytype` 用于设置学位类型和学习形式。当前示例是专业学位（专硕）版，`ezinfo.cfg` 中已注释列出切换为学术学位（学硕）时需联动修改的字段。

上游仓库是学硕模板，其封面/内封的基本结构仍保留在本项目中；但学硕蓝绿色外封、白色校徽资源和“学科/学科专业”标签已被当前专硕实现替代，因此不能只改一个字段就完整还原学硕版式。`bjutthesis.cls` 在这些切换点旁保留了说明注释。正式提交前，请按本人培养类型和学校最新样式核对。
